frappe.pages['Project Commercial'].on_page_load = function(wrapper) {
	...
	$("<div class='prj_commer-settings' id ='main-div'></div>").appendTo(page.main);
...
Project = Class.extend({
	init: function(wrapper) {
		this.wrapper = wrapper;
		this.body = $(this.wrapper).find(".prj_commer-settings");
		this.filters = {};
		this.make();
		this.refresh();
	},
	...
	make: function() {
		...
		$('#new').click(function(){
			frappe.route_options = {"customer_name": me.filters.customer.input.value }
			new_doc("Project Commercial");
		})
	...
	render: function() {
		...
		if(me.filters.customer.input.value.length!=null){
			frappe.call({
				...
				callback: function(r) {
					if(r.message.final_data.length){
						// var numPages=Math.ceil(r.message['final_data'].length/records_per_page)
